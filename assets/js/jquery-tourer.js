(function( $ ){
    ...
    function createNavigation(steps, n) {
        var tourNavigationHtml = '<div class="tour-nav-container">' +
        '<span class="tour-step-count"><b class="tour-current-step">1</b>/' + n + '</span>' +
        '<ul class="tour-nav">' +
            '<li><a href="javascript:void(0)" class="tour-prev">&#171; Previous</a></li>' +
            '<li><a href="javascript:void(0)" class="tour-next">Next &#187;</a></li>' +
        '</ul></div><a href="javascript:void(0)" class="tour-close">Close</a>';
        ...
        steps.each(function (index) {
            ...
            var nav = $(tourNavigationHtml).find('.tour-next').addClass(nextClass).end().find('.tour-prev').addClass(prevClass).end().find('.tour-current-step').html(stepNumber).end().appendTo(step.children('.tour-content'));
        });
    }
